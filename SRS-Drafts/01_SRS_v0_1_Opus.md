...
| CON-3 | MVP 월 인프라 비용은 Vercel Hobby 플랜 등 완전 무료 티어(Free Tier) 환경 내에 수용되도록 설계 | 비용 | PRD 5-3, 사용자 피드백 |
...
| **EXT-SYS-04 (환율)** | 마지막 성공 환율 캐시 반환 + 보조 환율 소스 조회 | Vercel KV 캐시(TTL 15min) 또는 보조 환율 크롤링 소스 사용 | 최대 1시간 이내 데이터 | "환율은 [HH:MM] 기준입니다 (지연 반영 중)." 환율 옆에 경고 아이콘 표시 |
...
| **Super-Calc API** | Route Handler (`/api/v1/compare`) | 성분명, 채널 리스트, 환율 | PRICE_SNAPSHOT 배열 (1일 단가 정렬) | 응답 <= 2초 (p95) |
| **Badge API** | Route Handler (`/api/v1/badges`) | ingredient_id | BADGE 배열 | 캐시 TTL: 24시간 |
...
| **Super-Calc Module** | Route Handler | 멀티채널 가격 조회, 환율 적용, 1일 단가 정규화 | p95 <= 2,000ms |
| **Badge Module** | Route Handler | 식약처 공전 기반 뱃지 판정, 일상어 번역 | p95 <= 1,000ms |
| **Search Module** | Route Handler | 성분/제품 검색, 자동완성 | p95 <= 1,000ms |
| **OG Generator** | Route Handler | `@vercel/og` 기반 동적 OG 이미지 생성 | p95 <= 1,500ms |
...
| **Price Sync Cron** | Vercel Cron | 일 1회 외부 API 배치 호출, PRICE_SNAPSHOT 갱신 | Vercel Cron (일 1회) |
| **Channel Adapters** | 라이브러리 | iHerb/쿠팡/네이버 API 호출 추상화 (전략 패턴) | 채널 추가 시 어댑터 1개 추가 |
| **Supabase DB** | BaaS (PostgreSQL) | 제품, 성분, 가격, 제보 데이터 영구 저장 | RPO <= 24h |
| **Vercel KV** | KV Store | 뱃지 캐시 (TTL 24h), 환율 캐시 (TTL 15min) | Free: 30K 요청/월 |
| **Supabase Storage** | Object Storage | 라벨 이미지, OG 이미지 저장 | Free: 1GB |
...
| ID | 요구사항 | 측정 지표 | 기준 | Source |
|---|---|---|---|---|
| **REQ-NF-019** | MVP 월간 인프라 비용은 Vercel Hobby 플랜 등 클라우드 단위 완전 무료 티어(Free Tier) 환경 내에 수용되도록 구조를 보장한다. | 월간 비용 | 월 0원 (인프라 비용) | CON-3, 피드백 |
...
|---|---|---|---|---|---|---|---|
| INT-API-01 | Super-Calc API | `GET /api/v1/compare` | GET | `ingredient`, `dosage`, `channels[]` | `PRICE_SNAPSHOT[]` (1일 단가 정렬) | Route Handler | 3채널 병렬 조회 |
| INT-API-02 | Badge API | `GET /api/v1/badges` | GET | `product_id` | `BADGE[]` + 일상어 번역 | Route Handler | 캐시 TTL: 24시간 |
| INT-API-03 | OG Image Generator | `GET /api/v1/share/og-image` | GET | 비교 결과 데이터 (Query) | OG 이미지 | Route Handler | `@vercel/og` 동적 생성 |
| INT-API-04 | 제품 검색 | `GET /api/v1/search` | GET | `query`, `category` | 제품 목록 | Route Handler | 서버 컴포넌트 직접 호출 가능 |
| INT-API-05 | 오류 제보 | `(Server Action)` | POST | `FormData` | 접수 상태 | Server Action | 스팸 필터링 포함 |
| INT-API-06 | 제품 등록 요청 | `(Server Action)` | POST | `FormData` | 접수 상태 | Server Action | — |
| INT-API-07 | 환율 조회 (내부) | `(Server Action)` | — | `currency` | 매매기준율 | Next.js Cache + KV | 15분 캐시 (`revalidate: 900`) |
...
| **SuperCalcModule** | Route Handler | 멀티채널 가격 조회, 환율 적용, 1일 단가 정규화·정렬 | `compare()`, `handleChannelFailure()` |
...
sequenceDiagram
    ...
    participant FXCache as Vercel KV (환율)
...
sequenceDiagram
    ...
    participant Cache as Vercel KV (뱃지)
    participant DB as Supabase DB
