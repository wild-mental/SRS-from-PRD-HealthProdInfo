...
| # | 범위 항목 | 설명 |
|---|---|---|
| IS-1 | F1. 실시간 1일 단가 정규화 엔진 | iHerb, 쿠팡, 네이버 3채널 대상 |
...
| CON-3 | MVP 월 인프라 비용은 무료 티어(Free Tier) 활용을 원칙으로 하되, 최대 월 10만 원 이하로 통제 | 비용 | PRD 5-3 (MVP 비용 현실화) |
...
| ID | 가정 내용 | 검증 방안 | 시한 |
|---|---|---|---|
| ASM-1 | iHerb/쿠팡 파트너스 API가 제품 성분 메타데이터까지 제공한다 | PoC(V3)에서 실제 API 응답 필드 확인 | MVP 착수 전 |
...
| ASM-5 | 환율 API(한국수출입은행 등)의 안정적 운영 및 15분 이내 갱신 | SLA 확인 + 폴백 환율 소스 확보 | MVP 착수 전 |
| ASM-6 | 카카오 Link API의 현 정책(외부 딥링크 허용)이 최소 6개월 유지 | 카카오 개발자 문서 변경 모니터링 | 지속 |

#### 1.2.5 Contingency Plans (비상 대응 계획)

##### CP-1. 제휴 API 성분 메타데이터 미제공 시 대안 데이터 소스 (ASM-1 실패 시)

iHerb/쿠팡 파트너스 API가 성분 메타데이터를 제공하지 않거나 데이터가 불충분할 경우, 다음 대안 데이터 소스를 우선순위에 따라 활용한다.
...
| 3 | **공개 건기식 성분 데이터베이스 (Open FDA, DSLD 등)** | 벌크 다운로드 + 정규화 | 해외 제품(iHerb) 성분 정보 (FDA Dietary Supplement Label Database) | 영문 기준 데이터. 한글 매핑 테이블 구축 필요. 국내 제품은 미포함 |
| 4 | **수동 입력(Crowd-sourcing)** | 관리자 직접 입력 + 사용자 제보(F4 Data Trust System 활용) | 대안 1~3으로 커버 불가한 제품 | 48시간 SLA 기반 오류 수정 프로세스와 통합. 초기 데이터 구축 비용 발생 |
...
##### CP-2. 카카오 Link API 정책 변경 시 우회 조치 방안 (ASM-6 실패 시)
...
| **Super-Calc Engine** | 멀티채널 실시간 1일 단가 정규화 및 비교 엔진 |
...
| # | 외부 시스템 | 유형 | 역할 | 제약 사항 |
|---|---|---|---|---|
| EXT-SYS-01 | iHerb Affiliate API | 외부 REST API | 제품 가격(USD), 재고, 성분표, 딥링크 URL 제공 | Rate Limit TBD (V3 PoC 확인 필요), 수수료: 신규 10% / 기존 5% |
| EXT-SYS-02 | 쿠팡 파트너스 API | 외부 REST API | 가격(KRW), 딥링크 URL, 제품 메타데이터 제공 | Rate Limit: 일 10,000건 (추정), 수수료: 3% |
| EXT-SYS-03 | 식약처 건강기능식품 공공 데이터 API | 외부 공공 API | 기능성 인정 내용, 일일 섭취량, 주의사항 제공 | 갱신 주기: 월 1회 |
| EXT-SYS-04 | 환율 API (한국수출입은행 등) | 외부 REST API | 매매기준율 제공 | 갱신 주기: 15분 |
| EXT-SYS-05 | 카카오 Link API | 외부 JS SDK | 카카오톡 공유 카드 생성·전송 | 일 발송 제한 확인 필요. 정책 변경 시 CP-2 우회 전략 적용 |
...
|---|---|---|---|---|
| **EXT-SYS-01 (iHerb)** | 마지막 수집 성공 시점의 `PRICE_SNAPSHOT` 캐시 데이터 반환 | `PRICE_SNAPSHOT` 테이블 (최근 수집분) | 최대 24시간 이내 데이터 (일 1회 배치 수집 보장) | "iHerb 가격은 [YYYY-MM-DD HH:MM] 기준입니다. 실시간 가격은 확인 중입니다." 인라인 표시 |
| **EXT-SYS-02 (쿠팡)** | 마지막 수집 성공 시점의 `PRICE_SNAPSHOT` 캐시 데이터 반환 | `PRICE_SNAPSHOT` 테이블 (최근 수집분) | 최대 24시간 이내 데이터 (일 1회 배치 수집 보장) | "쿠팡 가격은 [YYYY-MM-DD HH:MM] 기준입니다. 실시간 가격은 확인 중입니다." 인라인 표시 |
| **EXT-SYS-03 (식약처)** | 사전 벌크 수집한 식약처 공전 로컬 DB 사용 | `INGREDIENT` 테이블의 `mfds_status`, `mfds_claim` 필드 (월 1회 갱신 시 전량 동기화) | 식약처 공전 갱신 주기(월 1회)와 동일 — 실질적 품질 저하 없음 | 별도 안내 불요 (로컬 DB가 공전 원문과 동일 수준) |
| **EXT-SYS-04 (환율)** | 마지막 성공 환율 캐시 반환 + 보조 환율 소스 조회 | Redis 캐시(TTL 15min) → 실패 시 네이버 금융 환율 페이지 파싱 또는 최종 캐시 값 사용 | 최대 1시간 이내 데이터 | "환율은 [HH:MM] 기준입니다 (지연 반영 중)." 환율 옆에 경고 아이콘 표시 |
| **EXT-SYS-05 (카카오)** | CP-2 우회 전략 즉시 적용 (URL 복사 폴백 UI + 대체 공유 채널) | 해당 없음 (공유 기능 자체 우회) | 해당 없음 | "카카오톡 공유가 일시적으로 불가합니다. URL을 복사하여 공유해 주세요." 토스트 표시 |
...
| API | 유형 | 입력 | 출력 | 제약 사항 |
|---|---|---|---|---|
| **iHerb Affiliate API** | 외부 (REST) | 제품 ID, 키워드 | 가격(USD), 재고, 성분표, 딥링크 URL | Rate Limit TBD, 수수료: 신규 10% / 기존 5% |
...
| **Badge 내부 API** | 내부 (REST) | ingredient_id | BADGE 배열 | 캐시 TTL: 24시간 |
...
sequenceDiagram
    ...
    par 병렬 채널 조회
        API->>iHerb: GET /products?keyword=vitaminD+1000IU
        API->>Coupang: GET /products?keyword=비타민D+1000IU
        API->>FX: GET /exchange-rate?currency=USD
    end
    
    iHerb-->>API: 가격(USD), 성분표, 딥링크
    Coupang-->>API: 가격(KRW), 딥링크
    FX-->>API: 매매기준율

    API->>API: 1일 복용량 기준 원화 단가 정규화
    API->>API: 배송비 + 관세 포함 최종가 산출
    API->>API: 단가 기준 오름차순 정렬
    API->>DB: PRICE_SNAPSHOT 저장
    API-->>Web: 정렬된 단가 목록 (PRICE_SNAPSHOT[])
    Web-->>User: 1일 단가 비교 결과 표시 (<= 2초)

    Note over User,DB: 장애 채널 발생 시 정상 채널 결과만 표시 + 장애 안내 인라인 노출
```
...
sequenceDiagram
    ...
    participant Web as 모바일 웹 앱
    participant OG as OG 이미지 생성 서비스
    participant Kakao as 카카오 Link API
    actor Receiver as 수신자

    User->>Web: "카톡 공유" 버튼 탭
    Web->>OG: POST /api/v1/share/og-image (비교 결과 데이터)
    OG-->>Web: OG 이미지 URL

    Web->>Kakao: sendDefault (OG image, title, desc, 딥링크)
    ...
    alt 카카오 API 정상
        ...
        Web-->>User: "URL 복사" 폴백 UI + 네이버 밴드/문자 대체 버튼
    end
    ...
    Web-->>Receiver: 제휴 커머스 결제 페이지 딥링크 이동
```
...
| UC-02 | 멀티채널 1일 단가 비교 조회 | C1 | REQ-FUNC-001~006 |
...
> 프론트엔드 모바일 웹, API Gateway, MSA 형태의 백엔드 서비스, 외부 시스템으로 구성된 컴포넌트 아키텍처이다.
...
flowchart TB
    ...
    subgraph Services ["Backend Microservices"]
        ...
        subgraph CoreEngines ["Core Engines"]
            SC["Super-Calc Engine\n/api/v1/compare\n단가 정규화/정렬"]
            BE["Badge Engine\n/api/v1/badges\n식약처 뱃지 판정"]
        end
        subgraph SupportServices ["Support Services"]
            SS["Search Service\n/api/v1/search\n자동완성/검색"]
            OG["OG Image Generator\n/api/v1/share/og-image\n동적 이미지 생성"]
            RS["Report Service\n/api/v1/reports\n오류 제보 접수"]
            RW["Reward Service\n포인트/배지 지급"]
...
| 컴포넌트 | 유형 | 책임 | SLA / 제약 |
|---|---|---|---|
| **Mobile Web App** | 프론트엔드 | 사용자 인터페이스, 이벤트 트래킹 | LCP <= 2,500ms |
| **API Gateway** | 인프라 | 라우팅, 인증, Rate Limiting, TLS Termination | 전 구간 TLS 1.2+ |
| **Super-Calc Engine** | 백엔드 MSA | 멀티채널 가격 조회, 환율 적용, 1일 단가 정규화 | p95 <= 2,000ms |
| **Badge Engine** | 백엔드 MSA | 식약처 공전 기반 뱃지 판정, 일상어 번역 | p95 <= 1,000ms, 캐시 TTL 24h |
| **Search Service** | 백엔드 MSA | 성분/제품 검색, 자동완성 | p95 <= 1,000ms |
| **OG Image Generator** | 백엔드 MSA | 공유 카드용 동적 OG 이미지 생성 | p95 <= 1,500ms |
| **Report Service** | 백엔드 MSA | 오류 제보 접수, 스팸 필터링 | p95 <= 3,000ms |
| **Reward Service** | 백엔드 MSA | 제보 보상 포인트/배지 지급 | 수정 후 1시간 이내 |
| **Notification Service** | 백엔드 MSA | 푸시 알림 발송 (제보 결과, 가격 하락 등) | 트리거 후 1시간 이내 |
| **Redis Cache** | 데이터 | 뱃지 캐시 (TTL 24h), 환율 캐시 (TTL 15min) | — |
| **Product DB** | 데이터 | 제품, 성분, 가격, 제보 데이터 영구 저장 | RPO <= 24h |
| **S3 Storage** | 데이터 | 라벨 이미지, OG 이미지 저장 | — |
...
|---|---|---|---|---|
| **REQ-FUNC-001** | 시스템은 사용자가 영양소명(예: 비타민D 1000IU)을 입력하면 iHerb, 쿠팡, 네이버 3개 채널에서 해당 제품 목록을 병렬 조회한다. | M | Story 1, F1 | **Given** 사용자가 영양소 검색어를 입력한 상태 **When** "1일 단가 비교" 버튼을 탭하면 **Then** 3개 채널에 대한 병렬 API 조회가 실행되고, 조회 완료까지 로딩 인디케이터가 표시된다. |
| **REQ-FUNC-002** | 시스템은 각 제품의 1일 권장 복용량을 기준으로 원화(KRW) 단가를 자동 산출한다. 산출 공식: `(제품 가격 × 환율) ÷ (총 복용 횟수)`. | M | Story 1, F1 | **Given** 채널별 제품 목록이 조회된 상태 **When** 단가 정규화 엔진이 실행되면 **Then** 각 제품의 1일 복용량 기준 원화 단가가 산출되며, 산출 결과가 소수점 첫째 자리까지 표시된다. |
| **REQ-FUNC-003** | 시스템은 한국수출입은행 등 환율 API로부터 15분 이내 갱신된 매매기준율을 적용하여 USD→KRW 환산을 수행한다. 결과 화면에 "환율 기준 시각"을 표시한다. | M | Story 1 AC2, F1 | **Given** 사용자가 iHerb(USD) 제품과 쿠팡(KRW) 제품을 동시 비교하는 상태 **When** 결과 화면이 로드되면 **Then** 각 가격에 적용된 환율과 환율 기준 시각이 명시적으로 표시되며, 환율 정확도 오차가 ±0.5% 이내이다. |
...
| **REQ-FUNC-005** | 시스템은 산출된 1일 단가를 기준으로 오름차순 정렬하여 결과를 표시한다. | M | Story 1 AC1, F1 | **Given** 모든 채널의 1일 단가가 산출 완료된 상태 **When** 결과 화면이 렌더링되면 **Then** 1일 단가 기준 오름차순(최저가 상위)으로 정렬 표시되며, 전체 응답 시간이 2초(p95) 이내이다. |
| **REQ-FUNC-006** | 시스템은 단가 비교 API 요청에 대해 p95 응답 시간 2,000ms 이내, 실패율 1.0% 미만을 보장한다. | M | Story 1 AC1 | **Given** 동시 접속 200명 조건하에서 **When** 단가 비교 API 요청이 수행되면 **Then** p95 응답 시간이 2,000ms 이내이고, 실패율이 1.0% 미만이다. |
| **REQ-FUNC-007** | 시스템은 등록된 채널 중 일부(예: iHerb)의 API가 타임아웃 또는 5xx 오류를 반환할 경우, 정상 응답 채널의 결과만 표시하고 장애 채널에 "현재 [채널명] 가격을 가져올 수 없습니다. 마지막 수집: HH:MM" 인라인 안내를 표시한다. | M | Story 1 AC4, F1 | **Given** 3개 채널 중 1개 이상이 타임아웃/5xx 오류를 반환하는 상태 **When** 사용자가 1일 단가 비교를 요청하면 **Then** 정상 채널 결과는 정상 표시되고, 장애 채널에는 안내 메시지가 500ms 이내에 표시된다. 부분 결과 제공 성공률 >= 99%. |
...
| **REQ-FUNC-009** | 시스템은 각 비교 결과 행에 해당 채널의 제휴 구매 링크(딥링크)를 표시하며, 사용자가 탭 시 해당 채널의 결제 페이지로 이동한다. | M | Story 1, F1 | **Given** 단가 비교 결과가 표시된 상태 **When** 사용자가 특정 제품의 구매 링크를 탭하면 **Then** 해당 채널(iHerb/쿠팡)의 제휴 딥링크를 통해 결제 페이지로 이동한다. |
...
| **REQ-FUNC-017** | 시스템은 "카톡 공유" 버튼 탭 시 비교 결과의 핵심 지표(1일 단가, 뱃지, 성분 비교)가 포함된 Open Graph 이미지를 동적 생성한다. | M | Story 3 AC1, F3 | **Given** 사용자가 제품 비교 결과 화면에 있을 때 **When** "카톡 공유" 버튼을 탭하면 **Then** OG 이미지가 포함된 카카오톡 공유 카드가 1.5초(p95) 이내에 생성되며, 실패율 1% 미만이다. |
...
| **REQ-FUNC-021** | 시스템은 카카오 Link API 장애 시 자동으로 "URL 복사" 폴백 UI를 표시한다. URL 복사 시 "링크가 복사되었습니다" 토스트 알림을 제공하고, 네이버 밴드·문자 대체 공유 버튼을 노출한다. | M | Story 3 AC4, F3 | **Given** 카카오 Link API가 타임아웃 또는 오류를 반환하는 상태 **When** 사용자가 "카톡 공유" 버튼을 탭하면 **Then** 1초 이내에 "URL 복사" 폴백 UI가 표시되고, 복사 완료 시 토스트 알림이 뜨며, 대체 공유 버튼이 노출된다. 폴백 경로 공유 성공률 95% 이상. |
...
| **REQ-FUNC-026** | 시스템은 오류 수정 완료 시 제보자에게 푸시 알림("귀하의 제보로 수정되었습니다")을 발송하고 리워드(포인트/배지)를 지급한다. | M | Story 4 AC3, F4 | **Given** 제보한 오류가 검증·수정 완료된 상태 **When** 수정이 반영되면 **Then** 수정 후 1시간 이내에 푸시 알림이 발송되고 리워드가 지급된다. |
...
| **REQ-FUNC-037** | 시스템은 사용자가 관심 등록한 제품의 가격 하락 시 Push 알림을 발송한다. | S | F9 | **Given** 사용자가 특정 제품에 가격 알림을 등록한 상태 **When** 해당 제품의 1일 단가가 이전 대비 하락하면 **Then** 사용자에게 Push 알림("[제품명] 1일 단가가 XX원으로 하락했습니다")이 발송된다. |
...
|---|---|---|---|---|---|
| **REQ-NF-001** | 단가 비교 API의 응답 시간은 p95 기준 2,000ms 이내이다. | p95 응답 시간 | <= 2,000ms | 동시 접속 200명, 피크 500명 | PRD 5-1, Story 1 AC1 |
...
| **REQ-NF-006** | 시스템은 동시 접속 200명(피크 500명) 조건에서 모든 성능 기준을 충족한다. | 동시 접속 처리 | 200명(피크 500명) | MVP MAU 2,200명 기준 | PRD 5-1 |
| **REQ-NF-007** | 부하 테스트를 출시 전 1회 및 월 1회 수행한다. k6 또는 Locust 사용. 동시접속 200/500명 시나리오 각 15분 실행. | 테스트 주기 | 출시 전 1회 + 월 1회 | p95/p99/에러율 기록 | PRD 5-1 |
...
|---|---|---|---|---|
| **REQ-NF-009** | 월간 서비스 가용성은 99.5% 이상이다 (월 다운타임 3.6시간 이하). | 월간 업타임 | >= 99.5% | PRD 5-2 |
...
| **REQ-NF-013** | 데이터 백업은 일 1회 수행하며, RPO는 24시간 이하이다. | 백업 주기, RPO | 일 1회, RPO <= 24h | PRD 5-2 |
...
| ID | 요구사항 | 측정 지표 | 기준 | Source |
|---|---|---|---|---|
| **REQ-NF-019** | MVP 월간 인프라 비용은 클라우드 Free Tier 활용을 원칙으로 하되, 최대 월 10만 원 이하로 통제한다. | 월간 비용 | 무료~최대 10만 원 | PRD 5-3 (현실화), CON-3 |
...
| ID | 요구사항 | 측정 지표 | 기준 | Source |
|---|---|---|---|---|
| **REQ-NF-021** | 시스템은 API 응답 시간, 에러 코드, 크롤링 성공/실패를 CloudWatch/Datadog으로 로깅한다. p95 > 3초 또는 5xx > 1% 시 Slack 알림을 발송한다. | 로그 수집율, 알림 지연 | p95 > 3s or 5xx > 1% 시 즉시 Slack 알림 | PRD 5-4 |
...
| **REQ-NF-023** | 환율 API 장애, 제휴 API 연결 끊김, SLA 48시간 초과 제보 발생 시 PagerDuty를 통해 Tier 1 담당자에게 즉시 알림한다. | 알림 지연 | 즉시 (PagerDuty) | PRD 5-4 |

#### 4.2.6 Scalability / Maintainability (확장성/유지보수성)

| ID | 요구사항 | 측정 지표 | 기준 | Source |
|---|---|---|---|---|
| **REQ-NF-024** | 시스템은 신규 외부 채널(예: 아마존) 추가 시 기존 코드 변경 없이 채널 어댑터 모듈만 추가하여 확장 가능한 플러그인 아키텍처를 사용한다. | 채널 추가 시 수정 파일 수 | 어댑터 모듈 1개 추가로 한정 | 아키텍처 설계 원칙 |
...
| PRD 5-1 | REQ-NF-007 | TC-NF-007 | Performance |
...
|---|---|---|---|---|---|---|---|
| EXT-API-01 | iHerb Affiliate API | `https://api.iherb.com/v1/products` | GET | `keyword`, `product_id` | 가격(USD), 재고, 성분표, 딥링크 URL | TBD (V3 PoC 확인) | 수수료: 신규 10% / 기존 5% |
| EXT-API-02 | 쿠팡 파트너스 API | `https://api.coupang.com/v2/products` | GET | `keyword`, `category` | 가격(KRW), 딥링크 URL, 제품 메타 | 일 10,000건 (추정) | 수수료: 3% |
| EXT-API-03 | 식약처 건강기능식품 공공데이터 | `https://openapi.mfds.go.kr/v1/hfoods` | GET | `ingredient_name`, `cert_no` | 기능성 인정 내용, 일일 섭취량, 주의사항 | 공공 API 정책 준수 | 갱신 주기: 월 1회 |
| EXT-API-04 | 환율 API (한국수출입은행) | `https://www.koreaexim.go.kr/api/exchangeRate` | GET | `currency_code` | 매매기준율 | 공공 API 정책 준수 | 갱신 주기: 15분 |
| EXT-API-05 | 카카오 Link API | Kakao JS SDK `Kakao.Link.sendDefault()` | JS SDK | `content` (OG image, title, desc), `buttons` | 카카오톡 공유 메시지 | 일 발송 제한 확인 필요 | — |

#### 6.1.2 내부 API

| # | API | Endpoint | HTTP Method | 입력 파라미터 | 출력 | SLA | 비고 |
|---|---|---|---|---|---|---|---|
| INT-API-01 | Super-Calc API | `GET /api/v1/compare` | GET | `ingredient`, `dosage`, `channels[]` | `PRICE_SNAPSHOT[]` (1일 단가 정렬) | p95 <= 2,000ms | 3채널 병렬 조회 |
| INT-API-02 | Badge API | `GET /api/v1/badges` | GET | `product_id` | `BADGE[]` + 일상어 번역 | p95 <= 1,000ms | 캐시 TTL: 24시간 |
| INT-API-03 | OG Image Generator | `POST /api/v1/share/og-image` | POST | 비교 결과 데이터 | OG 이미지 URL | p95 <= 1,500ms | 동적 이미지 생성 |
| INT-API-04 | 제품 검색 API | `GET /api/v1/search` | GET | `query`, `category` | 제품 목록 | p95 <= 1,000ms | 자동 완성 포함 |
| INT-API-05 | 오류 제보 API | `POST /api/v1/reports` | POST | `product_id`, `field_name`, `reported_value`, `correct_value`, `evidence` | 접수 확인, `report_id` | p95 <= 3,000ms | 스팸 필터링 포함 |
| INT-API-06 | 제품 등록 요청 API | `POST /api/v1/product-requests` | POST | `ingredient_name`, `description` | 접수 확인, `request_id` | p95 <= 2,000ms | — |
| INT-API-07 | 환율 조회 (내부 캐시) | `GET /api/v1/exchange-rate` | GET | `currency` | 매매기준율, 기준 시각 | p95 <= 200ms | 15분 캐시 |
...
| `source_channel` | STRING | NOT NULL | 데이터 소스 채널 (iHerb, 쿠팡, 네이버) |
...
| `original_price` | FLOAT | NOT NULL | 원래 가격 (원통화 기준) |
| `currency` | STRING | NOT NULL | 통화 코드 (USD, KRW) |
| `exchange_rate` | FLOAT | — | 적용 환율 (KRW 제품은 NULL) |
...
| `badge_type` | STRING | NOT NULL, ENUM | 뱃지 유형: APPROVED / CAUTION / NOT_APPROVED |
...
| `product_id` | STRING | FK → PRODUCT, NOT NULL | 대상 제품 ID |
| `image_url` | STRING | NOT NULL | 라벨 이미지 URL (S3 등 스토리지) |
...
| `status` | STRING | NOT NULL, ENUM | 처리 상태: SUBMITTED / REVIEWING / RESOLVED / REJECTED |
...
classDiagram
    ...
    class SuperCalcEngine {
        <<Service>>
        +compare(ingredient, dosage, channels) PriceSnapshot[]
        +fetchChannelPrices(channels) Map
        +normalizeDaily(price, servings) Float
        +applyShippingAndCustoms(snapshot) PriceSnapshot
        +sortByDailyCost(snapshots) PriceSnapshot[]
        +handleChannelFailure(channel) PartialResult
    }
    ...
    BadgeEngine ..> Ingredient : queries
    BadgeEngine ..> Badge : creates
    ReportService ..> ErrorReport : manages
    ReportService ..> User : notifies
```
...
| **SuperCalcEngine** | Service | 멀티채널 가격 조회, 환율 적용, 1일 단가 정규화·정렬 | `compare()`, `handleChannelFailure()` |
| **BadgeEngine** | Service | 식약처 공전 기반 뱃지 판정, 금지 표현 검증, 일상어 번역 | `resolveBadges()`, `validateProhibitedExpressions()` |
| **ReportService** | Service | 오류 제보 접수, 스팸 필터링, 보상 지급, 알림 발송 | `submitReport()`, `filterSpam()`, `grantReward()` |
...
sequenceDiagram
    ...
    Web->>Calc: GET /api/v1/compare?ingredient=vitaminD&dosage=1000IU&channels=iherb,coupang,naver
    Web-->>User: 로딩 인디케이터 표시

    Calc->>FXCache: GET 환율 (USD/KRW)
    
    alt 캐시 Hit (15분 이내)
        FXCache-->>Calc: 매매기준율 + 기준 시각
    else 캐시 Miss
        Calc->>FX: GET /exchange-rate?currency=USD
        FX-->>Calc: 매매기준율
        Calc->>FXCache: 캐시 저장 (TTL 15min)
    end

    par 3채널 병렬 조회
        Calc->>iHerb: GET /products?keyword=vitaminD+1000IU
        Calc->>Coupang: GET /products?keyword=비타민D+1000IU
        Calc->>Naver: GET /products?keyword=비타민D+1000IU
    end

    alt iHerb 정상 응답
        iHerb-->>Calc: 가격(USD), 성분표, 딥링크 URL
    else iHerb 장애 (Timeout/5xx)
        iHerb-->>Calc: Error
        Calc->>Calc: 장애 채널 마킹 + 마지막 수집 시각 조회
    end

    Coupang-->>Calc: 가격(KRW), 딥링크 URL
    Naver-->>Calc: 가격(KRW), 딥링크 URL

    Calc->>Calc: 1일 복용량 기준 원화 단가 정규화
    Note over Calc: 공식: (제품가 x 환율) / 총 복용횟수
    Calc->>Calc: 배송비 + 관세 포함 최종가 산출
    Calc->>Calc: 오름차순 정렬 (1일 단가 기준)
    Calc->>DB: PRICE_SNAPSHOT[] 저장

    Calc-->>Web: 정렬된 PRICE_SNAPSHOT[] + 장애 채널 정보
    Web-->>User: 단가 비교 결과 표시 (p95 <= 2초)
    
    alt 장애 채널 존재
        Web-->>User: "현재 [채널명] 가격을 가져올 수 없습니다. 마지막 수집: HH:MM" 인라인 표시
    end
...
sequenceDiagram
    ...
    participant LabelStore as 라벨 아카이브 스토리지
...
sequenceDiagram
    ...
    participant Push as Push 알림 서비스
    ...
    alt 제보 유효 (수정 필요)
        ...
        DB-->>Push: 수정 완료 트리거
        Push->>User: "귀하의 제보로 수정되었습니다" 푸시 알림 (수정 후 1시간 이내)
        
        Reward->>DB: 리워드(포인트/배지) 지급
        Reward->>User: 리워드 지급 알림
    else 제보 무효
        Admin->>DB: ERROR_REPORT status - REJECTED (사유 기록)
        Admin->>Push: 제보자에게 사유 안내
        Push->>User: "확인 결과 현재 데이터가 정확합니다. [사유]" 알림
    end
...
sequenceDiagram
    ...
    participant Web as 모바일 웹 앱
    participant OGService as OG 이미지 생성 서비스
    participant Storage as 이미지 스토리지
    participant Kakao as 카카오 Link API
    participant Analytics as Mixpanel
    actor Receiver as 수신자
    ...
    Web->>OGService: POST /api/v1/share/og-image
    Note over OGService: 포함 데이터: 1일 단가, 뱃지, 성분 비교 핵심 지표
    OGService->>OGService: OG 이미지 동적 렌더링
    OGService->>Storage: 이미지 업로드
    Storage-->>OGService: 이미지 CDN URL
    OGService-->>Web: OG 이미지 URL (p95 <= 1.5초)
    ...
    alt 카카오 API 정상
        ...
        Web-->>Sender: 네이버 밴드, 문자 대체 공유 버튼 노출
...
| **Closed Beta 1** | C1 타겟 (iHerb 파워유저, 건기식 커뮤니티) | 30명 | F1 정확도/속도 검증, TTC 측정 | Beta 단계 |
